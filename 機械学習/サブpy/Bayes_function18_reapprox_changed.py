...
class Bayes_Function(Q_H):
    ...
    def __init__(self):
        ...
        self.Limit={"V1max":1,"V1min":1,"V2max":1,"V2min":1,"phimax":360,"phimin":360,\
                    "MWwidthmax":0.05,"MWwidthmin":0.05,\
                    "MWfreqmax":10,"MWfreqmin":10,"twmax":0.5,"twmin":0.5} #変更する範囲
        self.C_best=0
        self.C_best_i=0
    ...
    def Exp_limit(self,j):
        #実験パラメータを範囲内に収める
        for i,p in enumerate(self.RangeC):
    ...
    def UtilIG_bayes_risk_one(self):
        ...
        for k in range(self.n_exp(self.exp_flag)):
            #最もらしいms=0にいた回数
            num=self.d*np.transpose(self.ptable[0],(1,0))[k].reshape(self.x_approx.shape[0],1)
            num=np.round(self.Mean(self.w_approx,num))
            
            #実験設計Ckで実験したms=0の確率
            pjk=np.transpose(self.ptable[0],(1,0))[k].reshape(self.x_approx.shape[0],1)
            
            #尤度の算出、重みの更新
            L=binom.pmf(num,n=self.d,p=pjk)
            w_new=L*self.w_approx
            
            #ベイズリスクの算出
            x_infer=self.Mean(self.w_approx,self.x_approx)
            x_infer_new=self.Mean(w_new,self.x_approx)
    ...
    def UtilIG_bayes_risk_all(self):
        ...
        for i in range(2):
            ...
            for k in range(self.n_exp(self.exp_flag)):
                ...
                num=self.d*np.transpose(self.ptable[i],(1,0))[k].reshape(self.x_approx.shape[0],1)
                num=np.round(self.Mean(self.w_approx,num))
                pjk=np.transpose(self.ptable[i],(1,0))[k].reshape(self.x_approx.shape[0],1)
                L=binom.pmf(num,n=self.d,p=pjk)
                w_new=L*self.w_approx
                x_infer=self.Mean(self.w_approx,self.x_approx)
                x_infer_new=self.Mean(w_new,self.x_approx)
    ...
    def Update(self): #ベイズ推定を行う関数 引数(self,Ci)
        ...
        L=np.empty([self.n_particles(),1])
        counter=0
        
        #重みの小さいパーティクルの重みは更新しない(1を代入する)
        for j in range(self.x_approx.shape[0]):
            if j in self.delist_par and self.i != 0:
                counter=counter+1
                L[j][0]=1
            L[j+counter][0]=temp[j]
        self.w=L*self.w #重みの更新
        if np.sum(self.w)==0:
            self.w=np.ones([self.n_particles(),1])
        else:
...
class Bayes_parallel(Bayes_Function):
    ...
    def Prob_Lookup_parallel(self):
        ...
        if self.exp_select=="all":
            ...
            data_rabi=[(self.Expsim,self.x_approx[i],self.C[0][j]) for j in range(self.C[0].shape[0]) for i in range(self.x_approx.shape[0])]
            ptable_rabi=p.map(self.wrapper_Expsim, data_rabi)
            ptable_rabi=np.array(ptable_rabi).reshape(1,len(ptable_rabi)).reshape(self.x_approx.shape[0],self.n_exp("rabi")) #テーブルに変換
            
            #ラムゼー干渉についてテーブル作成
            self.exp_flag="ramsey"
            #data_ramsey = [(self.Expsim,self.x[i],self.C[1][j]) for i in range(self.x.shape[0]) for j in range(self.C[1].shape[0])]
            data_ramsey = [(self.Expsim,self.x_approx[i],self.C[1][j]) for i in range(self.x_approx.shape[0]) for j in range(self.C[1].shape[0])]
            ptable_ramsey=p.map(self.wrapper_Expsim, data_ramsey)
            ptable_ramsey=np.array(ptable_ramsey).reshape(1,len(ptable_ramsey)).reshape(self.x_approx.shape[0],self.n_exp("ramsey")) #テーブルに変換
            
            #各テーブルをまとめる
            self.ptable=[ptable_rabi,ptable_ramsey]
        
        else:
            data=[(self.Expsim,self.x_approx[i],self.C[0][j]) for i in range(self.x_approx.shape[0]) for j in range(self.C[0].shape[0])]
            self.exp_flag=self.exp_select
            self.ptable=p.map(self.wrapper_Expsim,data)
            self.ptable=[np.array(self.ptable).reshape(1,len(self.ptable)).reshape(self.x_approx.shape[0],self.n_exp(self.exp_select))] #テーブルに変換
        p.close()
        p.terminate()