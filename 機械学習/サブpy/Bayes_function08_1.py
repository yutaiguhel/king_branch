...
class Bayes_Function(Q_H):
    ...
    def Entropy(self):
        return -self.ptable_C*np.log2(self.ptable_C)-(1-self.ptable_C)*np.log2(1-self.ptable_C)
    ...
    def UtilIG(self): #量子操作の効用を計算する関数
        ...
        """
        if self.ptable_mode=="cross":
            ent_table = self.Entropy()#-self.ptable_C*np.log2(self.ptable_C) #エントロピーテーブル（各要素は各々の実験でのエントロピー)
