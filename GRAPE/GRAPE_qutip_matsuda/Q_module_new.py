...
#Hamilton(pulse,freq,Ome1,Ome2,fai1,fai2,theta,H0)
def Hamilton_GAUSSIAN(n_pi,pulse_time,freq,Ome1_rate,Ome2_rate,phi1,phi2,theta,t_div,H0):
    #pulse_hem = 200/1000.0#pulse_time/2.0#30/1000.0
    """
    Ome*∫exp(-a*t**2)dt = Ome*sqrt(pi/a) == n_pi-Ome*(pulse_time-2*pulse_hem)
    2*pulse_hem = 6.5*sigma = 6.5/sqrt(2*a)
    a = (1/2)*(6.5/(2*pulse_hem))**2
    Ome == n_pi/sqrt(pi/a)
    """
    width = 4
    a = (1/2.0)*(width/(pulse_hem))**2
    print("a = "+str(a))
    print("sigma = "+str(width/sqrt(2*a)))
    temp = sqrt(pi/a)+pulse_time-2*pulse_hem
    Ome = n_pi/2.0/temp    
    n_flat = int(round((pulse_time-2*pulse_hem)/t_div))    
    n_gauss0 = int(round(pulse_hem/t_div))
    n_gauss1 = int(round(pulse_time/t_div))-n_flat-n_gauss0  
    n_tslot = n_gauss0+n_flat+n_gauss1                  
    flat = Ome*ones(int(round((pulse_time-2*pulse_hem)/t_div)))
    t_g0 = linspace(t_div,t_div*n_gauss0,n_gauss0)
    t_g1 = linspace(t_div,t_div*n_gauss1,n_gauss1)
    gauss0 = Ome*exp(-a*(t_g0-pulse_hem)**2)
    gauss1 = Ome*exp(-a*(t_g1-pulse_hem)**2)[::-1]
    t_list = linspace(t_div,t_div*n_tslot,n_tslot)
    Ome1 = hstack((gauss0,flat,gauss1))*Ome1_rate#/sin(theta)/sqrt(2)*(-cos(theta)+sin(theta)) 
    Ome2 = hstack((gauss0,flat,gauss1))*Ome2_rate#/sin(theta)/sqrt(2)
    plt.plot(hstack((gauss0,gauss1)))
    plt.show()
    plt.plot(t_list,Ome1)
    plt.grid(b="on")
    plt.show()
    #ハミルトニアン
    H_list = []
    U_list = []
    for i in range(n_flat+n_gauss0+n_gauss1):
        H_div = Hamilton(0,freq,Ome1[i],Ome2[i],phi1,phi2,theta,H0)            
        U_div = (-2*pi*1j*H_div*t_div).expm()
        H_list = H_list+[H_div]
        U_list = U_list+[U_div]
...
def Hamilton_GAUSSIAN0(n_pi,pulse_time,freq,phi1,phi2,theta,t_div,H0):
    """
    Ome*∫exp(-a*t**2)dt = Ome*sqrt(pi/a) == n_pi
    pulse_time = 6*sigma = 6/sqrt(2*a)
    a = (1/2)*(6/pulse_time)**2
    Ome == n_pi/sqrt(pi/a)
    """
    a = (1/2.0)*(6.5/pulse_time)**2
    Ome = n_pi/sqrt(pi/a)    
    n_tslot = int(round(pulse_time/t_div))
    t_list = linspace(t_div,t_div*n_tslot,n_tslot)
    Ome1 = Ome*exp(-a*(t_list-pulse_time/2.0)**2)     
    Ome2 = zeros(n_tslot)
    plt.plot(t_list,Ome1)
    plt.grid(b="on")
    plt.show()
