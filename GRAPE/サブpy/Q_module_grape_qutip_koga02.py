...
class Grape_funcs(Q_H):
    def __init__(self):
        ...
        #stateGRAPEに必要なパラメータ
        self.state_init = tensor(zero,(zero+plus1+minus1)/sqrt(3),Hc)
        self.state_goal = tensor(H,(zero+plus1+minus1)/sqrt(3),Hc)
...
class Grape_funcs_for_single_wire(Grape_funcs):   
    ...
    def main(self):
        self.params()
        self.Hamiltonian_calc()
        count=0
        while self.fid_err > self.fid_err_tol:
            self.pulse_shape = self.make_init_pulse() 
